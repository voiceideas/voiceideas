...
npx supabase functions deploy organize --no-verify-jwt --project-ref "$PROJECT_REF"
npx supabase functions deploy share-idea --no-verify-jwt --project-ref "$PROJECT_REF"
npx supabase functions deploy transcribe --no-verify-jwt --project-ref "$PROJECT_REF"
